...
with open('/data/czyd/gradio_ui/评测数据/gsm8k.jsonl','r',encoding='utf-8') as file:
    for line in file:
        obj = json.loads(line)
        gsm8k_list.append(obj)

with open('/data/czyd/gradio_ui/评测数据/math.jsonl','r',encoding='utf-8') as file:
    for line in file:
        obj = json.loads(line)
        math_list.append(obj)
with open('/data/czyd/gradio_ui/评测数据/mmlu.jsonl','r',encoding='utf-8') as file:
    for line in file:
        obj = json.loads(line)
...
async def process_item_with_semaphore(item,semaphore):
    async with semaphore:
        try:
            ...
            print(e)
            item['prediction'] = """this line is error"""
            return item
...
async def process_test(file_list,save_path,max_concurrent_tasks):
        ...
        for i in file_list:
            ...
            task = asyncio.create_task(process_item_with_semaphore(i,semaphore))
            tasks.append(task)
...
#asyncio.run(process_test(math_list,'/data/czyd/gradio_ui/评测数据/result/math1.jsonl',50))
asyncio.run(process_test(gsm8k_list,'/data/czyd/gradio_ui/评测数据/result/gsm8k.jsonl',30))
#asyncio.run(process_test(mmlu_list,'/data/czyd/gradio_ui/评测数据/result/mmlu.jsonl',30))
